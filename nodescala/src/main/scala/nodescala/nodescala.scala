...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    for {chunk <- response} if (token.nonCancelled) exchange.write(chunk) else ()
    exchange.close
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val listenSubscription = listener.start()
    val handlerSubscription = Future.run() {
      cancelationToken =>
        Future {
          while (cancelationToken.nonCancelled) {
            listener.nextRequest().map{case (request, exchange) => (handler(request), exchange)}
              .map{case (response, exchange) => respond(exchange, cancelationToken, response)}
          }
...
object NodeScala {
  ...
  object Exchange {
    def apply(exchange: HttpExchange): Exchange = new Exchange {
      ...
      def write(s: String): Unit = os.write(s.getBytes)
  ...
  object Listener {
    class Default(val port: Int, val relativePath: String) extends Listener {
      ...
      def createContext(handler: Exchange => Unit): Unit = s.createContext(relativePath, new HttpHandler {
        def handle(httpxchg: HttpExchange): Unit = handler(Exchange(httpxchg))
      })

      def removeContext(): Unit = s.removeContext(relativePath)
    }
  }
