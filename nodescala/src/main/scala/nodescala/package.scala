...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] =
      fs match {
        case Nil => Future {Nil}
        case head::tail => head.zip(all(tail)).flatMap{case (h, t) => Future {h::t}}
      }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      def recurse(fs: List[Future[T]], p: Promise[T]): Promise[T] =
        fs match {
          case Nil => p
          case head::tail => {
            p.tryCompleteWith(head)
            recurse(tail, p)
          }
        }
      recurse(fs, Promise[T]).future
    }
  ...
  object CancellationTokenSource {
    ...
    def apply(): CancellationTokenSource = new CancellationTokenSource {
      ...
      val cancellationToken: CancellationToken = new CancellationToken {
        def isCancelled: Boolean = p.future.value.isDefined
      }
      def unsubscribe(): Unit = {
        p.trySuccess(())
      }
