...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(
    ...
    explored: Set[Block]
  ): Stream[(Block, List[Move])] = neighbors.filter{ case (block, _) => !explored.contains(block) }
  ...
  def from(
    ...
    val legalPaths = newNeighborsOnly(neighborsWithHistory(block, history), explored)
    val stream_list = for {(newBlock, _) <- legalPaths} yield from(legalPaths, explored + newBlock)
    val streams = stream_list.flatten
    legalPaths ++ streams
  }
