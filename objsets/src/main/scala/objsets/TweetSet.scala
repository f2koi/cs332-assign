...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val filteredAcc = if (p(elem)) acc else acc remove elem
    (left union right).filterAcc(p, filteredAcc)
  }
  ...
  def mostRetweeted: Tweet = {
    try {
      (left union right).filter(_.retweets > elem.retweets).mostRetweeted
    } catch {
  ...
  def descendingByRetweet: TweetList = {
    ...
    new Cons(mostRetweeted, this.filter(_ != mostRetweeted).descendingByRetweet)
  }
