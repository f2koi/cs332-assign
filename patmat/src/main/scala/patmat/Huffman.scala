...
object Huffman {
  ...
  def mergeCodeTables(left: CodeTable, right: CodeTable): CodeTable =
    left.map{case (ch, li) => (ch, 0::li)} ::: right.map{case (ch, li) => (ch, 1::li)}
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    @tailrec
    def quickEncode_tailrec(table: CodeTable, text: List[Char], acc: List[Bit]): List[Bit] =
      if (text.isEmpty) acc
      else quickEncode_tailrec(table, text.tail, acc ::: codeBits(table)(text.head))

    val table = convert(tree)
    quickEncode_tailrec(table, text, List())
  }
}
